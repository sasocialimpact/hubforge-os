...
export function GeekMode({ socket, connected }: { socket: Socket | null; connected: boolean }) {
  ...
  useEffect(() => {
    ...
    socket.on('engine:start', (p: any) => {
      ...
      pushEvent({ type: 'engine:start', engine: p.engine, iteration: p.iteration, title: `${p.engine} start${p.iteration ? ` iter ${p.iteration}` : ''}` })
    })

    socket.on('engine:done', (p: any) => {
      if (p.sessionId !== sessionIdRef.current) return
      setStatuses((prev) => {
        const next = { ...prev }
        const cur = next[p.engine as EngineId]
        ...
        if (p.iteration) {
          its = cur.iterations.map((it) => it.iteration === p.iteration ? { ...it, state: 'done' as EngineState } : it)
          if (!its.find((it) => it.iteration === p.iteration)) its.push({ iteration: p.iteration, state: 'done' })
        }
        next[p.engine as EngineId] = { state: 'done', iterations: its }
        return next
      })
      setActiveEngine((cur) => (cur === p.engine ? null : cur))
      pushEvent({ type: 'engine:done', engine: p.engine, iteration: p.iteration, title: `${p.engine} done${p.iteration ? ` iter ${p.iteration}` : ''}`, payload: { output: p.output } })
    })

    socket.on('engine:error', (p: any) => {
      if (p.sessionId !== sessionIdRef.current) return
      setStatuses((prev) => {
        const next = { ...prev }
        const cur = next[p.engine as EngineId]
        let its = cur.iterations
        if (p.iteration) its = cur.iterations.map((it) => it.iteration === p.iteration ? { ...it, state: 'error' as EngineState } : it)
        next[p.engine as EngineId] = { state: 'error', iterations: its }
        return next
      })
      pushEvent({ type: 'engine:error', engine: p.engine, iteration: p.iteration, title: `${p.engine} error`, payload: { error: p.error } })
    })

    socket.on('iteration:done', (p: any) => {
      if (p.sessionId !== sessionIdRef.current) return
      setCurrentIteration(p.iteration)
      pushEvent({ type: 'iteration:done', iteration: p.iteration, title: `iteration ${p.iteration} done`, payload: { qualityScore: p.qualityScore, thresholdMet: p.thresholdMet } })
    })

    socket.on('loop:complete', (p: any) => {
      if (p.sessionId !== sessionIdRef.current) return
      setRunning(false)
      setActiveEngine(null)
      setFinalRecord(p.record)
      setCurrentIteration(p.record?.iterations ?? 0)
      pushEvent({ type: 'loop:complete', title: 'loop complete', payload: { record: p.record } })
      socket.emit('memory:list', {})
    })

    socket.on('loop:error', (p: any) => {
      if (p.sessionId !== sessionIdRef.current) return
      setRunning(false)
      setActiveEngine(null)
      setError(p.error)
      pushEvent({ type: 'loop:error', title: 'loop error', payload: { error: p.error } })
    })
  ...
  const handleRun = useCallback(() => {
    ...
    setPackInfo(null)
    setRunning(true)
    socket.emit('run', { problem: problem.trim(), sessionId, outputTypes: ['strategy'] })
  }, [socket, connected, running, problem])

  const handleStop = useCallback(() => {
    if (socket) { socket.disconnect(); setTimeout(() => socket.connect(), 200) }
    setRunning(false)
    setActiveEngine(null)
  ...
  const handleClearMemory = useCallback(() => { socket?.emit('memory:clear', {}) }, [socket])
